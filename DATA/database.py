...
class Postgres:
    ...
    def getAccelerometr(self, id):
        with DataConn() as con:
            cur = con.cursor()
            cur.execute(
                f"select id,x,y,z from (select id,x,y,z from transfer_accelerometr order by id_req desc) as foo where id={id} Limit 1")
            rows = cur.fetchall()
    ...
    def getGyroscop(self, id):
        with DataConn() as con:
            cur = con.cursor()
            cur.execute(
                f"select id,x,y,z from (select id,x,y,z from transfer_gyro order by id_req desc) as foo where id={id} Limit 1")
            rows = cur.fetchall()
        return {rows[0][0]: [rows[0][1], rows[0][2]]}
    ...
    def getMeta(self, id):
        with DataConn() as con:
            cur = con.cursor()
            cur.execute(
                f"select id,volt from (select id,volt from transfer_meta order by id_req desc) as foo where id={id} Limit 1")
            rows = cur.fetchall()
    ...
    def insertgps(self, a):
        ...
        with DataConn() as con:
            cur = con.cursor()
            cur.execute(
                f"update transfer_gps set lat={x},lon={y} where id in (select id from transfer_gps order by id desc Limit 1) ")

    def gsmpower(self, id):
        with DataConn() as con:
            cur = con.cursor()
